...
Render의 Environment에 `GROQ_API_KEY`를 등록하면 무료 Groq API의
`openai/gpt-oss-20b` 모델이 인사, 자연스러운 표현, 후속 질문을 처리합니다.
실제 주가 수치는 기존 계산 엔진이 만든 검증 결과만 모델에 전달합니다.
키 누락, 무료 한도 초과 또는 API 장애 시 기존 계산형 Agent로 자동 전환됩니다.

선택 환경변수: `GROQ_MODEL` (기본값 `openai/gpt-oss-20b`)
